...
async function fetchPage({ clientId, token, offset }) {
  const query = `
    ...
    where platforms = (${PS2_PLATFORM_ID});
    sort name asc;
    limit ${PAGE_SIZE};
    offset ${offset};
  `;
...
for (let offset = 0; ; offset += PAGE_SIZE) {
  ...
  console.log(`Fetched ${page.length} PS2 titles at offset ${offset}`);
...
await writeFile("ps2TitleRef.json", `${JSON.stringify(deduped, null, 2)}\n`);
await writeFile(
  "ps2TitleRef.js",
  `export const ps2TitleRef = ${JSON.stringify(deduped, null, 2)};\n`
);

console.log(`Saved ${deduped.length} PS2 titles to ps2TitleRef.json and ps2TitleRef.js`);
